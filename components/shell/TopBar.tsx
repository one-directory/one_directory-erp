    ...
    <header className="sticky top-0 z-30 h-16 bg-white border-b border-slate-200/90 px-4 sm:px-6 flex items-center justify-between gap-4">
      ...
      <div className="flex-1 max-w-md hidden sm:block">
        <button
          ...
          className="w-full flex items-center justify-between px-3.5 py-1.5 text-xs text-slate-400 bg-slate-50 hover:bg-slate-100/80 border border-slate-200 rounded-xl transition-all shadow-2xs group"
        >
          <div className="flex items-center gap-2">
            <Search className="w-4 h-4 text-slate-400 group-hover:text-teal-600 transition-colors" />
            <span className="text-slate-500 font-normal">Search guests, bookings, leads...</span>
          </div>
          <kbd className="hidden lg:inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-mono text-slate-400 bg-white border border-slate-200 rounded-md shadow-2xs">
            <span>Ctrl</span>K
          </kbd>
        </button>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <button
          onClick={() => setIsCommandPaletteOpen(true)}
          className="sm:hidden p-2 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-800"
        >
        ...
        <div className="relative" ref={newMenuRef}>
          ...
          {isNewMenuOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-200 py-1.5 z-50 animate-in fade-in slide-in-from-top-2 text-xs">
              ...
              <button
                onClick={() => handleOpenAction('new-followup')}
                className="w-full px-3.5 py-2 text-left text-slate-700 hover:bg-slate-50 hover:text-teal-700 flex items-center gap-2.5 transition-colors cursor-pointer"
              >
        ...
        <div className="hidden sm:flex items-center gap-2.5 pl-2 border-l border-slate-200">
          ...
          <div className="text-left hidden lg:block">
            ...
            <p className="text-[10px] text-teal-600 font-medium">Ops Manager</p>
          </div>
        </div>
      </div>
    </header>
